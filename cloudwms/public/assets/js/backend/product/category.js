            ...
            var categoryIndex = new Vue({
                ...
                methods: {
                    ...
                    operation(type, id) {
                        ...
                        switch (type) {
                            case 'addCategory':
                                Fast.api.open("product/category/add", "新增分类", {
                                ...
                                break;
                            case 'editCategory':
                                Fast.api.open("product/category/edit?ids=" + id, "编辑分类", {
                                    callback: function (data) {
                                            if (data.type == 'edit') {
                                                that.getData(data.id)
                                            } else {
                            ...
                            case 'update':
                                ...
                                that.isAjax = false;
                                Fast.api.ajax({
                                    url: 'product/category/update?ids=' + that.tabsData[that.activeIndex].id,
                                    loading: false,
                                    type: 'POST',
                                    data: {
                ...
                methods: {
                    ...
                    deletecategory() {
                        ...
                        that.$confirm('此操作将删除该分类的所有相关数据, 是否继续?', '提示', {
                            ...
                            Fast.api.ajax({
                                url: 'product/category/del/ids/' + Config.row.id,
                                loading: true,
                                type: 'POST'
                            }, function (ret, res) {
                                Fast.api.close({
                    ...
                    submitForm(check) {
                        ...
                        this.$refs[check].validate((valid) => {
                            if (valid) {
                                if (that.optType != 'add') {
                                    Fast.api.ajax({
                                        url: 'product/category/edit?ids=' + Config.row.id,
                                        loading: true,
                                        type: "POST",
                                        data: {
