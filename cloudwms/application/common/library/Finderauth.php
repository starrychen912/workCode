...
use app\common\model\FinderUser;
...
class Finderauth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'email', 'phone', 'avatar'];
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            ...
            //初始化成功的事件
            Hook::listen("finderuser_init_successed", $this->_user);
    ...
    {
        // 检测用户名、邮箱是否存在
        if (FinderUser::getByUsername($username)) {
            $this->setError('Username already exist');
            return false;
        }
        if ($email && FinderUser::getByEmail($email)) {
        ...
        Db::startTrans();
        try {
            $user = FinderUser::create($params, true);

            $this->_user = FinderUser::get($user->id);
            ...
            //注册成功的事件
            Hook::listen("finderuser_register_successed", $this->_user, $data);
            Db::commit();
        } catch (Exception $e) {
    ...
    public function login($account, $password)
    {
        ...
        $user = FinderUser::get([$field => $account]);
        if (!$user) {
    ...
    public function logout()
    {
        ...
        //退出成功的事件
        Hook::listen("finderuser_logout_successed", $this->_user);
        return true;
    }
    ...
    public function changepwd($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        ...
        if ($this->_user->password == $this->getEncryptPassword($oldpassword, $this->_user->salt) || $ignoreoldpassword) {
            ...
            try {
                ...
                Token::delete($this->_token);
                //修改密码成功的事件
                Hook::listen("finderuser_changepwd_successed", $this->_user);
                Db::commit();
            } catch (Exception $e) {
                Db::rollback();
                $this->setError($e->getMessage());
    ...
    public function direct($user_id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                //登录成功的事件
                Hook::listen("finderuser_login_successed", $this->_user);
                Db::commit();
            } catch (Exception $e) {
                Db::rollback();
                $this->setError($e->getMessage());
