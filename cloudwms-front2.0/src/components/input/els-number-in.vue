  ...
  <div class="input-conected-content" :class="[{'input-conected-content-active':inputstatus},{'input-conected-content-error':error}]">
      <div class="Polaris-TextField_1spwi">
          <input class="Polaris-TextField__Input_30ock inputclasee" 
              ...
              @input="inputIn"
            >
          <div class="Polaris-TextField__Prefix_10fbz">{{name}}</div>
...
export default {
  ...
  methods:{
    inputketup(e){
      let vlaues = e.target.value.replace(/[^\-?\d.]/g,'')
    ...
    InputBlur(){
        ...
        this.$emit('InputBlur',Number(this.value).toFixed(2) != 'NaN' ? Number(this.value).toFixed(2):'')
    }
  },
  setup(props,content){
      const data = reactive({
