<template>
  <vite-dialog v-model="verification" title="新增" @ViteClose="ViteClose">
      <div class="dialogvisible_content">
        <el-form :model="ruleForm" :rules="ruleForm_rules" ref="rulefrom">
            ...
            <!-- <el-form-item label="类型：" prop="type" label-width="18%">
                  <el-select v-model="ruleForm.type" placeholder="请选择类型" @change="SelectType" clearable>
                      <el-option :label="item.name" :value="item.value" v-for="(item,index) in type_list" :key="index"></el-option>
                  </el-select>
              </el-form-item>
...
<script>
import { defineComponent, reactive, toRefs,inject, computed, ref, watch, nextTick } from 'vue'
...
import { Addtransaction ,GettransactionDetail,Updatetransaction} from '@/api/transaction.js'
...
export default defineComponent({
  ...
  setup(props,content) {
    ...
    const data = reactive({
      ...
      Togglestatus:true,
      //选择
      disabled:computed(() => {return !isNaLL(data.ruleForm.type)}),
      type_id_list:[],
      ruleForm_rules:{
        title: [
            { required: true, message: '请输入活动名称', trigger: 'blur' },
            { min: 3, max: 555555, message: '长度必须3 ', trigger: 'blur' }
        ],
        type: [
      ...
      GettransactionDetail({
        ...
        if(res.code === 1){
          ...
          data.ruleForm = {
              ...
              company_user_id:item.company_user_id,//发送人id
              to_company_user_id:item.to_company_user_id,//接收人id
              status:item.status,
              content:item.content
          }
          data.type_list.forEach(element => {
            if(element.value == item.type){
                SelectType(element.value)
            }
          });

          let re = /^http:\/\/.*?\/.*?\.jpg$/i;
          let list = [];
          item.prove_img.split(',').map(el =>{
              if(re.test(el)){
                list.push({
                  url:el,
                  src:el,
          ...
          data.flie_list = list;
          orderlistwangeditor.value.Init(item.content)
        } 
      })
    }
    //确定
    function submitForm(rulefrom){
      let image_urls;//图片整合
      console.log(data.flie_list)
      if(data.flie_list.length > 0 ){
        let imglist = [];
        let doname = /^(http[s]?:\/\/)?([^\/]+)(.*)/;
        if(data.flie_list.length > 0){
            for(let i=0; i<data.flie_list.length;i++){
              imglist.push(data.flie_list[i].src)
              // if(!doname.test(data.flie_list[i].src)){
              //   imglist.push(data.flie_list[i].src)
              // }
            }
        }
      ...
      rulefrom.validate((valid) =>{
        if(valid){
            Updatetransaction({
              ...
              company_user_id:data.ruleForm.company_user_id,//发送人id
              to_company_user_id:data.ruleForm.to_company_user_id,//接收人id
              status:data.ruleForm.status,
              prove_img:image_urls,
