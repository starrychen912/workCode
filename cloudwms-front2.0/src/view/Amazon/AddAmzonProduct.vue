    ...
    <el-container>
        ...
        <el-main>
          <div class="property-content flex align-center flex-wrap" ref="property">
              <template v-if="more_property">
                <div class="property-list flex align-center"  v-for="(item,index) in propertylist" :key="index" @click.stop="PropertyToggle(index,item)" >
                    <i class="el-icon-warning" v-if="item.status"></i>
                    <span>{{item.name}}</span>
          ...
          <div class="property_content">
              ...
              <information v-if="Activename == '重要信息'" ref="information" ></information>
          ...
          <div class="Save-btn">
            <el-button type="primary" @click="Submit">保存并完成</el-button>
...
export default {
    ...
    setup(props,content){
      ...
      function Submit(){
        information.value.init().then(res =>{
          console.log(res);
        })
      }
      return {
