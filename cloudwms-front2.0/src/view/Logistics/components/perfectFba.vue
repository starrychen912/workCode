  ...
  <div class="addfba_content">
    ...
    <div class="from-list">
      <el-form :model="ruleForm" ref="rulefrom" :rules="ruleForm_rules" :inlist="true">
        ...
        <el-form-item label="FBA发货仓库：" label-width="18%" prop="fba_warehouse" >
          <div class="form_list--input">
            <el-input v-model="ruleForm.fba_warehouse" clearable placeholder="请输入" :disabled="ruleForm.create_type == '1'"></el-input>
          </div>
        </el-form-item>
        <el-form-item label="FBA编码：" label-width="18%" prop="fba_num" >
          <div class="form_list--input">
            <el-input v-model="ruleForm.fba_num" clearable placeholder="请输入" :disabled="ruleForm.create_type == '1'"></el-input>
          </div>
        </el-form-item>

        <el-form-item label="包装类型：" label-width="18%" prop="logistics_status">
          <el-select v-model="ruleForm.logistics_status" placeholder="请选择活动区域" clearable>
            <el-option label="空运" value="1"></el-option>
            <el-option label="船运" value="2"></el-option>
          </el-select>
        </el-form-item>
...
import { fbaPlanAdd ,fbaDetail,fbaAdd} from '@/api/FBA.js'
...
export default {
  ...
  setup(props,content){
    ...
    function Get_ProductDeatil(){
      ...
      let newval = router.currentRoute.value
      newval.meta.title = route.query.title 

      store.dispatch('tagsView/UptodateView',{oldval:router.currentRoute.value,newval:newval})
      fbaDetail({
        fba_id:route.query.fba_id,
      }).then(res =>{
        if(res.code == 1){
          data.fba_id = res.data.id
          data.ruleForm = {
            title:res.data.name || '',
            market_id: parseInt(res.data.market_id) || '',
            shop_id:res.data.shop_id || '',
            warehouse_id:res.data.warehouse_id || '',
            packing_type:res.data.packing_type || '',
            create_type: res.data.create_type || '',
            mark:res.data.mark || '',
            product_id:res.data.order_item || [],
            logistics_status:res.data.logistics_status || '',
          },
          data.skus =  res.data.order_item
          editwangeditor.value.installText(res.data.mark)
        }
      })
    }
    ...
    function onSubmit(item){
      item.validate((valid, fields) => {
        if(valid){
          delete data.ruleForm.product_id
          fbaAdd({
            company_id:store.state.userinfo.userinfo.company_id,
            ...data.ruleForm,
            ...
            if(res.code == 1){
              ...
              store.dispatch('tagsView/delView',router.currentRoute.value).then(({visitedViews}) =>{
                router.push({path:'/Logisticsmanagement/FBA'})
