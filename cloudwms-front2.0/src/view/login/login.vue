 <template>
  <div class="container">
      <div class="container_title">2014 © 深圳町町信息科技有限公司 {{$version}}</div>
          ...
          <div class="conteiner_content--text" >
              ...
              <div class="avatar-login">
                    ...
                    <div class="button--btn">
                        <button class="b-btn-login" @click="Debounce" :disabled="loaddingstatus">
                            <i class="el-icon-loading" v-if="loaddingstatus"></i>
                            登录
                        </button>
                    </div>
              </div>
...
<script>import { reactive,ref,nextTick ,onMounted,computed, watch,getCurrentInstance,onUnmounted,toRefs} from "vue"
...
export default {
    setup(props,context){
        const {appContext:{config:{globalProperties:{$version}}}} = getCurrentInstance();
        
        const store = useStore();
        const router = useRouter();
        const ipt =ref();
        const Debounce = throttle(Login,10);
        ...
        function Login(){
            ...
            login({
              account:formLabelAlign.username,
              password:formLabelAlign.password  
            }).then(res => {
                data.loaddingstatus = false
                if(res.code === 1){
