<template>
    <!-- 弹出新增商品框 -->
    <el-dialog v-model="dialog_visible" :destroy-on-close="true"  width="80%" top="20px"  :title="title" :fullscreen="isFullscreen" :show-close="false" custom-class="dialog_productlist"  :lock-scroll="false" >
    <template #title>
      <div class="el-dialog_title">
        ...
        <div class="icon_list">
          ...
          <i class="el-icon-close" @click="Close('revise_dialog_visible')"></i>
        </div>
      </div>
    </template>
    <el-tabs v-model="activeName" @tab-click="handleClick" >
      <el-tab-pane label="基本信息" name="first" class="baseInfo">
        ...
        >
          ...
          <!-- <el-form-item
            ...
            class="colForm"
          >
            <el-input v-model="ruleForm.proSku"></el-input>
          </el-form-item> -->
          <el-form-item
            label="库存sku："
            prop="stockSku"
            class="colForm"
          >
            <el-input v-model="ruleForm.stockSku" placeholder="库存SKU" disabled></el-input>
          </el-form-item>
        ...
        <div class="addOnline">
          <el-table :data="addOnline" ref="table">
            ...
            <el-table-column label="成本价（元）" align="center" min-width="130">
              <template #header>
                <div class="purchasePrice_title">成本价（元）</div>
              </template>
              <template #default="scope">
                <div class="purchasePrice flex" :class="{'is_active':rules_cost}">
                  <el-input
            ...
            <el-table-column label="零售价（元）" align="center" min-width="130">
              ...
              <template #default="scope">
                <div class="purchasePrice flex" :class="{'is_active':rules_retailPrice}">
                  <el-input
            ...
            <el-table-column label="原价（元）" align="center" min-width="130">
              ...
              <template #default="scope">
                <div class="purchasePrice flex" :class="{'is_active':rules_CompareatPrice}">
                  <el-input
            ...
            <el-table-column label="平台价（元）" align="center" min-width="130">
              <template #header>
                <div class="purchasePrice_title">市场价（元）</div>
              </template>
              <template #default="scope">
                <div class="purchasePrice " :class="{'is_active':rules_MarketPrice}">
                  <el-input
            ...
            <el-table-column label="包装重量（G）" align="center" min-width="150">
              <template #default="scope">
                <el-input
                  class="proWeigh"
                  v-model="scope.row.packageWeigh"
                ></el-input>
              </template>
            </el-table-column>
            <el-table-column
        ...
        <div class="textarea">
          <div class="textarea_title">备注：</div>
          <!-- <el-input
            type="textarea"
            placeholder="请输入内容"
             ...
             <el-form-item v-for="(item,index) in extra" :key="index">
              ...
              <div class="flex"> 
                ...
                <div class="extra_button">
                  <el-button type="primary" v-if="index == extra.length-1" @click="Addextra">添加</el-button>
                  <el-button type="danger" v-if="extra.length != 1" @click="Dleteextra(index)">删除</el-button>
                </div>
              </div>
            </el-form-item>
           </div>
      ...
      <el-tab-pane label="库存信息" name="third">
        ...
        <div class="addOnline">
          <div class="addOnline_title"><i class="el-icon-warning"></i>默认仓库是自动选择第一个仓库，在没有输入自定义库存数系统会默认为10。</div>
          <el-empty v-show="Stockinfo.length == 0">
             <el-button size="mini" type="primary" @click.prevent="addRow('库存信息')">新增</el-button>
          </el-empty>
          <el-table :data="Stockinfo" ref="table" v-show="Stockinfo.length > 0">
            <el-table-column
              label="序号"
              type="index"
              width="60"
            ...
            <el-table-column label="仓库" align="center">
              <template #default="scope">
                <el-select v-model="scope.row.warehouse_id" @change="Option_Change(scope.$index,$event)">
            ...
            <el-table-column label="最低安全库存数" align="center">
              <template #default="scope">
                <el-input
                  class="retailPrice"
                  v-model="scope.row.stock_minimum"
                ></el-input>
              </template>
            </el-table-column>
            <el-table-column label="最高安全库存数" align="center">
              <template #default="scope">
                <el-input
                  class="platformPrice"
                  v-model="scope.row.stock_highest"
                ></el-input>
              </template>
            </el-table-column>
            <el-table-column
      ...
      <el-tab-pane label="图片信息" name="fourth">
        <div class="pic_title"><i class="el-icon-warning"></i>默认图一为首图显示</div>
        <div class="pic">
                  ...
                  <div class="pic_upload-demo">
                    <ul class="el-upload-list el-upload-list--picture-card">
                        <li class="el-upload-list__item is-success">
                          <el-image fit="cover" style="width: 100%; height: 100%"  :src="element.src" :preview-src-list="flie_list"></el-image>
    ...
    <template #footer class="el-dialog-footer">
      ...
      <el-button @click="resetForm('ruleForm','ruleForm_extend')">重置</el-button>
      <el-button type="primary" @click="submitForm('ruleForm','ruleForm_extend')">确定</el-button>
    </template>
  </el-dialog>
...
import {AddingProduct,GetProductDetail,UpdateProduct} from '@/api/Addproduct'
...
export default {
      ...
      Stockinfo:[
            {
              warehouse_id:'默认',
              stock:10, //初期库存
              stock_minimum:10, //最高安全库存数 
              stock_highest:10, //最地区安全库存数
            }
      ],
      //扩展信息额外参数
      extra:[
  ...
  computed:{
    rules_cost:function(){
      return !isNaLL(this.addOnline[0].cost) && this.cost_status
    },
    rules_retailPrice:function(){
      return !isNaLL(this.addOnline[0].retail_price) && this.retailPrice_status
    },
    rules_CompareatPrice:function(){
      return !isNaLL(this.addOnline[0].compare_at_price) && this.CompareatPrice_status
    },
    rules_MarketPrice:function(){
      return !isNaLL(this.addOnline[0].market_price) && this.MarketPrice_status
    }
  },
  methods:{
    //库存选择
    ...
    Enlarge_IMG(item){
      ...
      if(doname.test(this.flie_list[i].url)){
        ...
        this.Enlarge_Dialog_IMG = (this.baseUrl+item.fullurl)
      }
    },
    //上传图片error
    UploadError(){
    ...
    //入库
    async OperateInventoryLog(){
        for(let i=0; i<this.Stockinfo.length;i++){
            operateInventoryLog({
                warehouse_id:this.Stockinfo[i].warehouse_id,
                inventory_sku:this.ruleForm.stockSku,
                in_quantity:this.Stockinfo[i].stock,
                out_quantity:'',
                min_quantity:this.Stockinfo[i].stock_minimum, //最低区安全库存数
                max_quantity:this.Stockinfo[i].stock_highest, //最高安全库存数 
                memo:'',
                ...
                if(res.code == 1){
                  ...
                  return shoElmessage(true,'保存失败','error')
                }
          })
        }
    },
    ...
    async  submitForm(formName,fromExtend) {
      let image_urls;//图片整合
      if(this.flie_list.length > 0 && this.upload_satus){
        let imglist = [];
        let doname = /^(http[s]?:\/\/)?([^\/]+)(.*)/;
        if(this.flie_list.length > 0){
            for(let i=0; i<this.flie_list.length;i++){
              if(!doname.test(this.flie_list[i].url)){
                imglist.push(this.flie_list[i].url)
              }
      ...
      //库存信息
      if(this.Stockinfo.length == 0 || this.Stockinfo[0].warehouse_id == '默认'){
        this.stock_status = true
         return shoElmessage(true,'请选择库存信息，不许默认','error')
      }

      let formNamecode = false;

      this.cost_status = true
      this.retailPrice_status = true  
      this.CompareatPrice_status = true
      this.MarketPrice_status = true
      ...
      await this.$refs[formName].validate((valid) => {
        if (valid) {
          if(this.rules_cost){
            ...
            return shoElmessage('el-icon-warning','请输入成本价','error');
          }
      ...
      await this.OperateInventoryLog();
      // console.log(this.upload_satus , this.Stockinfo.length)
      // if(this.upload_satus != this.Stockinfo.length) return false;
      //所有商品的参数


      let onlinelist = {}
      if(this.addOnline.length != 0 && this.addOnline.length == 1){
          onlinelist = Object.assign(this.addOnline[0].proSize,this.addOnline[0].packSize)
          onlinelist.weight = this.addOnline[0].proWeigh
          onlinelist.packageWeigh = this.addOnline[0].packageWeigh
          onlinelist.cost = this.addOnline[0].cost //成本价
          onlinelist.compare_at_price = this.addOnline[0].compare_at_price //零售价
          onlinelist.market_price = this.addOnline[0].market_price //平台价
          onlinelist.retail_price = this.addOnline[0].retail_price //零售价
          onlinelist.encoding = this.addOnline[0].code //条码
      }
      if(formNamecode){
        UpdateProduct({
          product_id:this.detail_data.id,
          company_id:this.$store.state.userinfo.userinfo.company_id, //必
          ...
          // cost:this.addOnline[0].purchasePrice, //成本价
          ...
          delete_image_ids:this.Delete_list.join(','),
          ...
          hs_code:'', //海关代码
           ...
           if(res.code == 1){
              ...
              this.Close('revise_dialog_visible');
           }else{
      ...
      GetProductDetail({
        ...
        if(res.code == 1){
              ...
              this.ruleForm = {
                ...
                brand:res.data.data.brand_id || '',
                supplier:res.data.data.supplier_id || '',
                proSku:res.data.data.sku || '',
                stockSku:res.data.data.inventory_sku || '',
              ...
              this.addOnline = [
                {
                  code: item.encoding,
                  proWeigh: item.weight,
                  proSize: { length: item.length, width: item.width, height: item.height },
                  packageWeigh: item.package_weight,
                  packSize: { package_length: item.package_length, package_width:item.package_width, package_height: item.package_height, },
                  cost: item.cost,
              ...
              //库存信息
              this.Stockinfo = res.data.data.inventory || []
    ...
    Cancal(){
      if(this.revise_detil){
         this.$confirm('你修改数据确定操作取消？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$emit('Close',{item:'revise_dialog_visible',dialog_visible:false})
          this.$message({
            type: 'success',
            message: '取消成功!'
          });
