<template>
  <vite-dialog v-model="dialog_visible"  top="20px" title="修改商品" @ViteClose="Close('编辑')">
              ...
              <template #default="scope">
                <div class="expand_props_list">
                      ...
                      <template #item="{ element }">
                        <div class="pic_upload-demo">
                          <ul
                            class="
                              el-upload-list
                              expand-upload-list--picture-card
                            "
                          >
                  ...
                  <div class="expand_stock_list">
                    ...
                    >
                      ...
                      <el-descriptions-item label="原价">{{
                        expand_list.compare_at_price
                      }}</el-descriptions-item>
                      <el-descriptions-item label="成本价">{{
                        expand_list.cost
                      }}</el-descriptions-item>
                      <el-descriptions-item label="市场价">{{
                        expand_list.market_price
                      }}</el-descriptions-item>
                      <el-descriptions-item label="零售价">{{
                        expand_list.retail_price
                      }}</el-descriptions-item>
                      <el-descriptions-item label="品牌名称">{{
                        expand_list.brand_name
                      }}</el-descriptions-item>
                      <el-descriptions-item label="序列号">{{
                        expand_list.encoding
                      }}</el-descriptions-item>
                      <el-descriptions-item label="商品类型">{{
                        expand_list.category_name
                      }}</el-descriptions-item>
                      <el-descriptions-item label="供应商">{{
                        expand_list.supplier_id
                      }}</el-descriptions-item>
                      <el-descriptions-item label="创建时间">{{
                        expand_list.create_time
                      }}</el-descriptions-item>
                      <el-descriptions-item label="商品状态">
                        <el-tag
                          :type="
                            expand_list.status == 0
                              ? 'success'
                              : expand_list.status == 1
                              ? 'info'
                              : expand_list.status == 2
                              ? 'warning'
                              : ''
                          "
                        >
                  ...
                  <div class="expand_stock_list_description">
                    <div class="expand_stock_list_description--title">
                      描述：
                    </div>
              ...
              <el-form-item v-for="(item, index) in extra" :key="index">
                ...
                <div class="flex">
                  ...
                  <div class="extra_button">
                    <el-button type="primary" size="mini" @click="Addextra()"
                      >添加</el-button
                    >
                    <el-button
                      type="danger"
  ...
  <!-- 搜索商品 -->
  <el-dialog
    v-model="product_dialog_visible"
    width="70%"
    top="15px"
    :title="title"
    custom-class="ecominfomanag_dialog"
  ...
  >
    ...
    >
      ...
      <el-table-column prop="sku" label="商品SKU" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="sku" label="仓库SKU" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="barcode" label="条码"> </el-table-column>
      <el-table-column prop="title" label="名称" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="category_name" label="类别" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="规格" show-overflow-tooltip>
        <template #default="scope">
          <span class="proSize size">{{ scope.row.width }}</span>
...
import { UpdateProduct } from "@/api/AmazonProduct";
...
export default {
  ...
  methods: {
    ...
    Init(product_id) {
      ...
      this.Delete_img_list = [];
      GetAmzonProductDetail({
        product_id: product_id,
      }).then((res) => {
        if (res.code == 1) {
          this.bullet = res.data.data;
          this.bullet.title = res.data.data.a_title;
          this.product_amazon_id = res.data.data.amazon_id;
          this.flie_list = res.data.data.images;
          this.selectTable.push(res.data.data);
          this.expand_list = res.data.data;
          this.expands.push(res.data.data.id);
    ...
    DELETE_IMG(item) {
      ...
      this.Delete_img_list.push(this.flie_list[index].id);
    ...
    resetForm(fromExtend) {
      ...
      this.$refs[fromExtend].validate((valid) => {
        if (valid) {
          UpdateProduct({
            product_amazon_id: this.bullet.amazon_id,
            title: this.bullet.title,
            description: this.bullet.description,
            bullet_point1: this.bullet.bullet_point1,
            bullet_point2: this.bullet.bullet_point2,
            bullet_point3: this.bullet.bullet_point3,
            bullet_point4: this.bullet.bullet_point4,
            bullet_point5: this.bullet.bullet_point5,
            image_urls: this.bullet.image_urls,
            delete_image_ids: this.Delete_img_list.join(","),
          }).then((res) => {
            if (res.code == 1) {
