...
<template>
  <!-- 弹框 -->
  <vite-dialog  v-model="dialog_visible" :title="title" @ViteClose="CloseDialog('revise_dialog_visible')">
    <!-- 表格 -->
    ...
    <div class="addOnline">
      <div class="textarea_title">采购信息：</div>
       <el-table :data="addOnlines" ref="table" :border="true" v-show="false">
            <!-- <el-table-column
              label="序号"
              type="index"
              width="60"
      ...
      <el-table :data="addOnline" ref="table" :border="true" >
        <el-table-column label="商品sku" prop="sku" >
          <template #default="scope">
            <div :class="{'active_error':this.sku_status && scope.$index == 0} ">
            <el-select
    ...
    <template #footer class="el-dialog-footer">
      <el-button @click="CloseDialog('revise_dialog_visible')">取消</el-button>
      <el-button @click="resetForm('ruleForm')">重置</el-button>
      <el-button type="primary" @click="PurchaseUpdate('ruleForm')">确定修改</el-button>
    </template>
  </vite-dialog>
</template>
<script>
    ...
    data() {
        return {
            ...
            rules_basic: {
                ...
                product_development_id: [
                    { required: false, message: "请选择商品", trigger: "change" }
                ],
                shop_id:[
    ...
    methods: {
        ...
        Init(id) {
            ...
            purchaseDetail({
              ...
              console.log(Detail)
               // 初始化基本
              if(parseInt(Detail.purchase_type) == 1){
                this.ruleForm = {
                      purchase_type: Detail.purchase_type,
                      purchase_date: (Detail.purchase_date * 1000),
                      warehouse_id: Detail.warehouse_id,
                      company_user_id: parseInt(Detail.company_user_id),
                      is_check: parseInt(Detail.is_check) ,
                      purchase_quantity: Detail.purchase_quantity  || '',
                      current_total_price: Detail.current_total_price  || '',
                      note: Detail.note  || '',
                      product_development_id:Detail.product_development_id || "",
                      title: Detail.title  || '',
                      category_id: Detail.category_id  || '',
                      brand_id: Detail.brand_id  || '',
                      supplier_id: Detail.supplier_id || '', //供应商
                      shop_id:Detail.shop_id,
                  };
                this.$refs.editwangeditor.installText(Detail.note);
                this.addOnline = Detail.order_item
              }else{
                this.$nextTick(() => {
                  this.ruleForm = {
                      purchase_type: Detail.purchase_type,
        ...
        CloseDialog(item) {
            this.$emit("CloseDialog", {
                item: item,
                revise_dialog_visible: this.dialog_visible,
            });
        ...
        delData(index, item) {
            console.log(item)
            this.delete_item_ids.push(item[index].item_id)
            item.splice(index, 1);
        },
        // 重置
        resetForm() {
        ...
        // 调用新增采购接口
        PurchaseUpdate(formName) {
            let item;
            if(parseInt(this.ruleForm.purchase_type) == 1){
              if(this.ruleForm.sku == ''){
                this.sku_status = true
              }
              delete this.ruleForm.brand_id
              delete this.ruleForm.supplier_id
              delete this.ruleForm.product_development_id
              delete this.ruleForm.category_id
              delete this.ruleForm.title
              this.ruleForm.purchase_date = this.ruleForm.purchase_date / 1000;
              item = {
                ...this.ruleForm
              }
           }else{
              ...
              item = {
                ...this.addOnlines[0].purchasePrice,
           ...
           let skus = [];
           let add_skus = [];
           this.addOnline.forEach(el =>{
             if(isNaLL(el.item_id)){
               skus.push({
          ...
          this.$refs[formName].validate((valid) => {
                if (valid && !this.sku_status) {
                    purchaseUpdate({
                        ...
                        add_skus:JSON.stringify(add_skus),
